...
function autoReply(customMessage) {
  ...
  if (checkinEmailThread) {
    // Check if script has already replied to this thread
    const hasReplied = replyToThread_(
      checkinEmailThread,
      myEmail,
      reply_message,
    );
    if (hasReplied === "This script replied earlier") {
      return hasReplied;
    }

    chatCheckin(reply_message);
    return "Replied and posted to chat.";
  } else {
...
function getDayDate_(today) {
  ...
  const dayLong = today
    ...
    .toLowerCase(); // pattern 1
  const monthSlashDate_num = today.toLocaleString("en-US", {
    month: "numeric",
    day: "numeric",
  }); // pattern 2
  const dayLongSpaceDateSlash_num = `${dayLong} ${monthSlashDate_num}`; // pattern 3
  const monthLongDateNum = today
  ...
  return [
    dayLong,
    monthSlashDate_num,
    dayLongSpaceDateSlash_num,
    dayLongCommaMonthLongDateNum,
    dayLongMonthLongDateNum,
  ];
}
